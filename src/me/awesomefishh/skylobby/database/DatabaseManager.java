package me.awesomefishh.skylobby.database;
...
public class DatabaseManager {
    ...
    public String getGameDataTable() {
        return "game_data";
    }
    ...
    public void checkPlayer(Player player) {
        ...
        try {
            ...
            for (String kit : plugin.getConfigManager().getKitConfig().getConfigurationSection("kits").getKeys(false)) {
                PreparedStatement ps = getConnection().prepareStatement("SELECT * FROM " + getKitTable() + " WHERE UUID='" + player.getUniqueId() + "' AND KIT='" + kit + "'");
                ResultSet rs = ps.executeQuery();
                if (!rs.next()) {
                    PreparedStatement insertPlayer = getConnection().prepareStatement("INSERT INTO " + getKitTable() + " (UUID,KIT,LEVEL) VALUES ('" + player.getUniqueId() + "','" + kit + "',1)");
                    insertPlayer.executeUpdate();
                }
            }
